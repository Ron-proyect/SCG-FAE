...
def cargar_entregas():
    if os.path.exists(ENTREGAS_FILE):
        ...
        df['Caso'] = df['Caso'].astype(str).str.strip()
        df['Fecha Envio Real'] = pd.to_datetime(df['Fecha Envio Real'], errors='coerce').dt.date
        df = df.dropna(subset=['Fecha Envio Real'])
        return df[df['Fecha Envio Real'] > pd.to_datetime('1980-01-01').date()]
